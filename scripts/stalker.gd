...
@export var lunge_speed := 14.0
@export var stalk_speed := 2.2
@export var acceleration := 6.0
@export var gravity := 20.0
@export var attack_distance := 1.5
@export var lunge_time := 0.4
@export var leash_distance := 80.0
@export var patience_time := 5.0
@export var hide_distance := 14.0
@export var noise_memory_time := 7.5
@export var sense_radius := 50.0
...
var _last_noise_position := Vector3.ZERO
...
func _update_attack(delta: float) -> void:
    ...
    var monster_pos := global_transform.origin
    var player_pos := _player.global_transform.origin
    var direction := (player_pos - monster_pos).normalized()
...
func _follow_path(delta: float, target_speed: float) -> void:
    ...
    var target := _path[_path_index] + Vector3(0, 0, 0)
...
func _choose_hiding_destination(player_pos: Vector3) -> void:
    var player_cell := _level.world_to_grid(player_pos)
    var target_cell := _level.get_random_cell_near(player_cell, 6, true, player_pos)
    _path = _level.find_path(_level.world_to_grid(global_transform.origin), target_cell)
    _path_index = 0

func _player_is_visible() -> bool:
    var from_pos := global_transform.origin + Vector3.UP * 1.6
    var to_pos := _player.global_transform.origin + Vector3.UP * 1.5
...
func _teleport_closer() -> void:
    var player_cell := _level.world_to_grid(_player.global_transform.origin)
    var spawn_cell := _level.get_random_cell_near(player_cell, 5, true, _player.global_transform.origin)
...
func _process(delta: float) -> void:
    ...
    if _state == "STALK" and _noise_timer > 0.0:
        var target_cell := _level.world_to_grid(_last_noise_position)
        _path = _level.find_path(_level.world_to_grid(global_transform.origin), target_cell)
        _path_index = 0
